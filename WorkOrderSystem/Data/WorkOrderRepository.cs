    ...
    public class WorkOrderRepository : IWorkOrderRepository
    {
        ...
        public async Task<IReadOnlyList<WorkOrder>> GetWorkOrdersByStatusAsync(string status)
        {
            return await _context
                .WorkOrders.Where(w => w.Status == status)
                .Include(w => w.Technician)
                .ToListAsync();
        }

        public async Task<IReadOnlyList<WorkOrder>> GetWorkOrdersByTechnicianIdAsync(
            int TechnicianId
        )
        {
            return await _context
                .WorkOrders.Where(w => w.TechnicianId == TechnicianId)
                .Include(w => w.Technician)
                .ToListAsync();
        }

        public async Task<bool> UpdateWorkOrderStatusAsync(int id, WorkOrderUpdateDto statusUpdate)
        {
            ...
            existingWorkOrder.Status = statusUpdate.Status;
